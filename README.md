...
**Multi-part RAR sets** (`name.part1.rar … name.partN.rar`) are treated as one
model: only the first volume is processed (it pulls in the rest), and the
`.partN` marker is stripped from the model name.
